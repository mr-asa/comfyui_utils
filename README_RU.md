...
- Фильтры списка: `?+` только включенные (linked), `?-` только выключенные (unlinked), `?*` все.
- При фильтре команды `a <n>`, `r <n>`, `i <n>` работают по номерам текущего отфильтрованного списка.
